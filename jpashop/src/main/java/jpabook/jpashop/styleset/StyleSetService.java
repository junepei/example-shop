...
public class StyleSetService {
    ...
    public LowestPriceCollection getLowestPriceCollection() {
        ...
        validateLowestPriceStyleSetProducts(styleSetProducts);
        return new LowestPriceCollection(styleSetProducts);
    }

    private void validateLowestPriceStyleSetProducts(List<StyleSetProduct> styleSetProducts) {
            ...
            if (StyleSetType.values().length > styleSetProducts.size()) {
                ...
                for (StyleSetType styleSetType : StyleSetType.values()) {
                    if(!savedStyleSetTypes.contains(styleSetType)) {
                        throw new InternalServerErrorException("StyleSetProduct의 최저가 설정이 잘 못 되었습니다. styleSetType : "
                                + styleSetType
                                + "의 상품이 없습니다."
                        );
                    }
                ...
                throw new InternalServerErrorException("StyleSetProduct의 최저가 설정이 잘 못 되었습니다. "
                        + problems.toString()
                        + "의 설정이 중복되었습니다."
                );
            }
